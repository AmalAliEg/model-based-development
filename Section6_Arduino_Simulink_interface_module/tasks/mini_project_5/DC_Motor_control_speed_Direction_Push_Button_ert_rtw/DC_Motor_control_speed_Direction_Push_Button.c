...
void DC_Motor_control_speed_Direction_Push_Button_step1(void) /* Sample time: [0.1s, 0.0s] */
{
   ...
   *  MATLABSystem: '<Root>/Analog Input'
   * */
  u0 = (real_T)((uint32_T)DC_Motor_control_speed_Direct_P.Gain_Gain *
  ...
  /* MATLABSystem: '<Root>/PWM' */
  MW_PWM_SetDutyCycle
    (DC_Motor_control_speed_Direc_DW.obj_i.PWMDriverObj.MW_PWM_HANDLE, u0);

  /* MATLABSystem: '<Root>/button status' */
  if (DC_Motor_control_speed_Direc_DW.obj_dl.SampleTime !=
      DC_Motor_control_speed_Direct_P.buttonstatus_SampleTime) {
   ...
   *  MATLABSystem: '<Root>/button status'
   * */
  DC_Motor_con_motorenergized(!c_value, &DC_Motor_control_speed_Direct_B.Merge,
    &DC_Motor_control_speed_Direct_B.Merge1,
    &DC_Motor_control_speed_Direct_P.motorenergized);

  /* End of Outputs for SubSystem: '<Root>/motor energized' */

  /* MATLABSystem: '<Root>/button status1' */
  if (DC_Motor_control_speed_Direc_DW.obj_d.SampleTime !=
      DC_Motor_control_speed_Direct_P.buttonstatus1_SampleTime) {
  ...
  DC_Motor_con_motorenergized(!c_value, &DC_Motor_control_speed_Direct_B.Merge,
    &DC_Motor_control_speed_Direct_B.Merge1,
    &DC_Motor_control_speed_Direct_P.motorenergized1);

  /* End of Outputs for SubSystem: '<Root>/motor energized1' */

  /* MATLABSystem: '<Root>/Digital Output to the motor' */
  u0 = rt_roundd_snf(DC_Motor_control_speed_Direct_B.Merge);
  if (u0 < 256.0) {
    if (u0 >= 0.0) {
      tmp = (uint8_T)u0;
    } else {
  ...
  u0 = rt_roundd_snf(DC_Motor_control_speed_Direct_B.Merge1);
  if (u0 < 256.0) {
    if (u0 >= 0.0) {
      tmp = (uint8_T)u0;
    } else {
