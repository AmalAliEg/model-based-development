...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) mini_project_1_M->Timing.clockTick0;
  mini_project_1_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(mini_project_1_M, 0);
  ...
  mini_project_1_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(mini_project_1_M->extModeInfo, &rtmGetTFinal
    (mini_project_1_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(mini_project_1_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(mini_project_1_M);
