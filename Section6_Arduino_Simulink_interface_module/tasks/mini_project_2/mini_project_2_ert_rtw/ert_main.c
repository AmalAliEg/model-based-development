...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) mini_project_2_M->Timing.clockTick0;
  mini_project_2_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(mini_project_2_M, 0);
  ...
  mini_project_2_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(mini_project_2_M->extModeInfo, &rtmGetTFinal
    (mini_project_2_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(mini_project_2_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(mini_project_2_M);
